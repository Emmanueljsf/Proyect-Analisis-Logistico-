...
def generar_reporte_salidas_excel(txt_universal: str, rango_fechas: list, opt_estado: str, usuario_emisor: str) -> bytes:
    """
    Genera un archivo Excel con un diseño de "tarjetas herméticas" para cada acta de salida, permitiendo una 
    visualización estructurada de los despachos. Incluye barrera de acceso para administradores y manejo de
    excepciones mediante logs estructurados.

    Parámetros:
    txt_universal (str): Filtro de búsqueda (p. ej., paciente, orden, insumo).
    rango_fechas (list): Lista con fechas de inicio y fin.
    opt_estado (str): Estado del acta (VÁLIDO/ANULADO).
    usuario_emisor (str): Nombre del usuario que genera el archivo.

    Retorna: bytes (Stream binario del archivo Excel) o b"" en caso de error o sin registros.
    """
    ...
    try:
        ...
        logging.error(f'{{"correlation_id": "{correlation_id}", "error": "{str(e)}", "modulo": "reporte_salidas_excel"}}')
...
def generar_reporte_salidas_pdf(txt_universal: str, rango_fechas: list, opt_estado: str, usuario_emisor: str) -> bytes:
    """
    Genera el reporte de auditoría en PDF estructurando de manera elegante las actas. Cada acta se encapsula
    herméticamente dentro de una sola tabla compacta. Incluye barrera de acceso para administradores y 
    manejo de excepciones mediante logs estructurados.
    ...
    Retorna: bytes (Stream binario del archivo pdf) o b"" en caso de error o sin registros.
    """    
    # Barrera de seguridad explícita
    if not es_administrador():
        st.error("Acceso denegado: Solo administradores pueden generar este reporte.")
    ...
    try:
        ...
        correlation_id = str(uuid.uuid4())
        logging.error(f'{{"correlation_id": "{correlation_id}", "error": "{str(e)}", "modulo": "reporte_salidas_pdf"}}')
        st.error(f"Error generando PDF. Reporte el código: [{correlation_id}]")
        return b""


# GENERADOR DEL ACTA DE PÉRDIDA POR CADUCIDAD
def generar_reporte_salida_caducidad_pdf(datos_despacho: dict, lista_insumos: list, usuario_emisor: str) -> bytes:
    """
    ...
    Parametros:
        datos_despacho (dict): Metadatos de la transacción. Debe contener:
            - orden_salida, paciente_destino, fecha, 
            lista_insumos (list):  Cada elemento debe ser un dict con:
            - nombre_insumo 
            - codigo_lote 
            - cantidad
        usuario_emisor: Nombre del operador o responsable técnico que realiza la baja.

    Returns: bytes: Stream binario del PDF generado listo para descarga o previsualización.
    """
    ...
    try:
        ...
        correlation_id = str(uuid.uuid4())
        logging.error(f'{{"correlation_id": "{correlation_id}", "error": "{str(e)}", "modulo": "reporte_salida_caducidad_pdf"}}')
        st.error(f"Error generando PDF. Reporte el código: [{correlation_id}]")
        return b""